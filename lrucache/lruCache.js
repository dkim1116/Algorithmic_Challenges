...
LRUCache.prototype.get = function (key) {
  for (var i = 0; i < this.storage.length; i++) {
    ...
    if(this.storage[i].hasOwnProperty(key)){
      if(this.storage[i][key]===null){
        return null;
      } else {
        var result = this.storage[i][key];
        var temp;
        temp = this.storage[i];
        this.storage[i] = this.storage[this.storage.length-1]
        this.storage[this.storage.length-1] = temp;
        // var result = this.storage.splice(i,1);
